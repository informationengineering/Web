...
function ChatModal({ open, onClose, currentUserId, targetUserId, targetUserName }) {
  ...
  useEffect(() => {
    if (open) {
      apiRequest(`/chat/${targetUserId}`)
        .then(res => {
          if (!res.ok) throw new Error(`狀態錯誤: ${res.status}`);
          return res.json();
        })
        .then(data => setMessages(data))
        .catch(err => console.error('取得訊息失敗:', err));
    }
  }, [open, targetUserId, apiRequest]);
            ...
            messages.map((msg, idx) => (
              <div
                ...
                style={{
                  ...
                  margin: "6px 0",
                  color: msg.sender_id === currentUserId ? "#384aa5" : "#555"
                }}
              >
